...
public:
    ...
    public:
        ...
        // for reconstruction
        NodeType* came_from; // *** Theta-star: same as 'grand-parent'
    ...
    public:
        ...
        inline bool isSegmentFree (NodeType &n1, NodeType &n2, CostType* c) // *** Theta-star specific
...
template <class AlgDerived, class NodeType, class CostType>
void ThetaStar::Algorithm<AlgDerived,NodeType,CostType>::search (void)
{
    ...
    while (!(node_heap_p->empty()))
    {
        ...
        for (auto it = thisNodeInHash_p->successors.begin(); it!=thisNodeInHash_p->successors.end(); ++it)
        {
            ...
            if (_this->isSegmentFree (*(thisNodeInHash_p->came_from), *this_neighbour_node_in_hash_p, &grandparentSegmentCost)) { // path 2
                //printf("Theta-star: grandparentSegmentCost = %f\n", grandparentSegmentCost);
                temp_g_val = thisNodeInHash_p->came_from->g_score + grandparentSegmentCost;
                if (temp_g_val < test_g_val) {
                    test_cameFrom = thisNodeInHash_p->came_from;
            ...
            if (test_g_val < this_neighbour_node_in_hash_p->g_score) {
                ...
                this_neighbour_node_in_hash_p->f_score = _this->getHeapKey (*this_neighbour_node_in_hash_p);
                this_neighbour_node_in_hash_p->came_from = test_cameFrom;
                this_neighbour_node_in_hash_p->lineage_data = test_cameFrom->lineage_data.next_generation();
...
template <class AlgDerived, class NodeType, class CostType>
void ThetaStar::Algorithm<AlgDerived,NodeType,CostType>::clear (unsigned int mode)
{
    NodeType* thisNodeInHash_p;
    ...
    /*else if (resetNodesInHash)
        for (int a=0; a<all_nodes_set_p->hash_table_size; ++a)
            for (int b=0; b<all_nodes_set_p->HashTable[a].size(); ++b) {
                all_nodes_set_p->HashTable[a][b]->clear_search_data();
        }*/
}
...
template <class AlgDerived, class NodeType, class CostType>
std::vector<NodeType*> ThetaStar::Algorithm<AlgDerived,NodeType,CostType>::reconstruct_pointer_path (NodeType n)
{
    _dosl_verbose_head(1);
...
template <class AlgDerived, class NodeType, class CostType>
    std::vector < std::unordered_map <NodeType*, CostType> >
        ThetaStar::Algorithm<AlgDerived,NodeType,CostType>::reconstruct_weighted_pointer_path (NodeType n)
{
