...
public:
    ...
    void print (std::string head="", std::string tail="\n") const {
        ...
        (this->g_score == std::numeric_limits<double>::max())? printf("INF") : printf("g_score = %0.8f \n", this->g_score);
...
template <class doslAlgorithm>
class cvPathPlanner : public doslAlgorithm::template Algorithm< cvPathPlanner<doslAlgorithm>, cvPPnode<doslAlgorithm>, double>
{
public:
    ...
    void find_path (cv::Point start, cv::Point goal, bool vis=false) {
        ...
        auto dosl_path = this->reconstruct_weighted_pointer_path (goalNode);
        double cost = 0.0;
                            ...
                            : GRAPH_TYPE(graph_type), VERTEX_LOCATION(vertex_location), HEURISTIC(heuristic_type)
    {
        ...
        this->all_nodes_set_p->reserve (ceil(WIDTH - 0 + 1));
