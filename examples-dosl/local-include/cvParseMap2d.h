...
public:
    ...
    // void update(void) -->
    void update (const cv::Mat& new_map=cv::Mat(), int computeRepPts=-1, int threshold=-1) { 
    ...
    void update (std::string fName, int computeRepPts=0, int threshold=DEFAULT_THRESHOLD) {
        map = cv::imread (fName, CV_LOAD_IMAGE_GRAYSCALE);
        update (cv::Mat(), computeRepPts, threshold);
    }
    // --------------------------------------
    
    cvParseMap2d() : repPtsComputed(false) { }
    cvParseMap2d(cv::Mat& new_map, int computeRepPts=0, int threshold=DEFAULT_THRESHOLD) { update (new_map, computeRepPts, threshold); }
    cvParseMap2d(std::string fName, int computeRepPts=0, int threshold=DEFAULT_THRESHOLD) { update (fName, computeRepPts, threshold); }
    ...
    cv::Mat getCvMat (MAP_COLOR_TYPE color_type = GRAYSCALE_MAP, PARSE_MAP_TYPE map_type = FREE_MAP) {
